...
const App: React.FC = () => {
  ...
  const startLogging = useCallback(async () => {
    ...
    try {
      ...
      // 1. Handle Android Native Picker (Ask-at-Start)
      if (hasAndroidInterface && (window as any).AndroidInterface.requestLogFile) {
        addDebugLog("LOGGING: Requesting Android System Picker...");
        
        // Define the callback that Android will call
        (window as any).onNativeLogFileReady = (success: boolean) => {
          if (success) {
            addDebugLog("LOGGING: Android Picker Success. Starting stream...");
            finalizeStartLogging(null, fileName, startDate, excelSerialDate);
          } else {
            addDebugLog("LOGGING: Android Picker Cancelled.");
          }
          delete (window as any).onNativeLogFileReady;
        };

        (window as any).AndroidInterface.requestLogFile(fileName);
        return; // Wait for callback
      }

      // 2. Handle Desktop File Picker
      if (hasFileSystemAPI) {
        try {
      ...
      // 3. Fallback to Memory Mode (Standard Mobile Browser)
      finalizeStartLogging(writable, fileName, startDate, excelSerialDate);
  ...
  // Helper to finish the start process after picker (or fallback)
  const finalizeStartLogging = (writable: any, fileName: string, startDate: Date, excelSerialDate: number) => {
    const hasAndroidInterface = typeof (window as any).AndroidInterface !== 'undefined';
    
    if (!writable && !hasAndroidInterface) {
    ...
    } else if (hasAndroidInterface && (window as any).AndroidInterface.appendLogData) {
      (window as any).AndroidInterface.appendLogData(header);
    } else {
  ...
  const stopLogging = useCallback(async () => {
        ...
        const triggerDownload = (data: string[], fileName: string, mimeType: string, delay: number = 0) => {
          setTimeout(() => {
            ...
            reader.onloadend = () => {
              ...
              // Check if we are in the Android app
              if ((window as any).AndroidInterface && (window as any).AndroidInterface.saveFile) {
                (window as any).AndroidInterface.saveFile(data.join('\n'), fileName);
                addDebugLog(`LOGGING: Native save triggered for ${fileName}`);
                return;
              }
    ...
    // Close Android Log File if active
    if ((window as any).AndroidInterface && (window as any).AndroidInterface.closeLogFile) {
      (window as any).AndroidInterface.closeLogFile();
    }
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (pendingFramesRef.current.length > 0) {
        ...
        if (isLogging && (traceWriterRef.current || isLoggingFallback || (window as any).AndroidInterface)) {
          try {
            const rows = batch.map((f) => {
              ...
              if ((window as any).AndroidInterface && (window as any).AndroidInterface.appendLogData) {
                (window as any).AndroidInterface.appendLogData(row + "\n");
              } else if (isLoggingFallback) {
                mobileLogBufferRef.current.push(row);
              }
              return row;
            });
