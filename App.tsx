...
const App: React.FC = () => {
  ...
  const exportFile = useCallback(async (data: string, fileName: string, mimeType: string = 'text/plain') => {
    ...
    if (android && android.saveFileWithPicker) {
      ...
      addDebugLog(`SYS: Triggered download for ${fileName}`);
    }
  }, [addDebugLog]);
  ...
  const handleSaveTrace = useCallback(async (isAuto: boolean = false) => {
    ...
    // Use a slight timeout to let the UI update the "Saving" state
    setTimeout(async () => {
      try {
  ...
  }, [frames, exportFile, addDebugLog]);
  
  const handleSaveDecoded = useCallback(async (isAuto: boolean = false) => {
    if (frames.length === 0) return;
    if (!isAuto) setIsSavingDecoded(true);
    addDebugLog(isAuto ? "AUTOSAVE: Generating Decoded Telemetry..." : "SYS: Pre-indexing Matrix for Decoded Export...");

    setTimeout(async () => {
      try {
        // Pre-index the DBC for O(1) lookups during the big loop
        const idToDbcMap = new Map<string, DBCMessage>();
        Object.entries(library.database).forEach(([decId, msg]) => {
          idToDbcMap.set(normalizeId(decId), msg);
        });

        // Determine which messages actually exist in the trace
        const uniqueIdsInTrace = new Set<string>();
        const sampleLimit = Math.min(frames.length, 100000); // Sample last 100k to determine headers efficiently
        const framesToScan = frames.slice(-sampleLimit);
        framesToScan.forEach(f => uniqueIdsInTrace.add(normalizeId(f.id.replace('0x', ''), true)));
        ...
        if (activeSignalMeta.length === 0) {
          if (!isAuto) addDebugLog("SYS: No defined signals detected in current buffer.");
        ...
        // Limit the heavy processing to last 100,000 frames to prevent browser crash
        const processBuffer = frames.length > 100000 ? frames.slice(-100000) : frames;
        
        processBuffer.forEach((frame, idx) => {
          ...
          if (signalNamesInMsg) {
            ...
            if (dbEntry) {
              signalNamesInMsg.forEach(sName => {
                ...
                lastKnownValues[sName] = val.split(' ')[0];
              });
              const row = [(frame.timestamp / 1000).toFixed(3), ...activeSignalMeta.map(s => lastKnownValues[s.name])].join(",");
              csvRows.push(row);
            }
          }
        });
        ...
        if (!isAuto) addDebugLog("SYS: Wide-format export complete.");
      } catch (e) {
  ...
  }, [frames, library, exportFile, addDebugLog]);
  ...
  }, [frames.length, autoSaveEnabled, handleSaveTrace, handleSaveDecoded]);
          ...
          <main className="flex-1 overflow-hidden relative flex flex-col min-h-0">
            {dashboardTab === 'link' ? (
              ...
              <LibraryPanel 
                ...
                latestFrames={latestFrames} 
                onSaveDecoded={() => handleSaveDecoded(false)} 
